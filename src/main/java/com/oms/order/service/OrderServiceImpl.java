...
public class OrderServiceImpl implements OrderService {
	@KafkaListener(topics = "kafka-user2", groupId = "group_id")
    public void consume(String message) {
        System.out.println("Added to cart: " + message);
	}
	...
	@Override
	public OrderPlacedDTO placeOrder(String buyerMode,List<ProductDTO> productList, List<CartDTO> cartList, OrderDTO orderDTO)
	...
	@Override
	public String discount(String orderId,String buyerId) throws OrderMsException {
		Optional<Order> optional = orderRepository.findByOrderId(orderId);
		Order order = optional.orElseThrow(()->new OrderMsException("Order does not exist"));
		Float a = order.getAmount();
		Integer points = (int) (a/100);
		Integer dis=(int)(points/4);
		order.setAmount(a-dis);
		//s=s.concat(t);
		//System.out.println(s);
		return String.valueOf(points);
	}
	public String changeOrderStatus(String orderId,String status) throws OrderMsException{
		Optional<Order> optional = orderRepository.findByOrderId(orderId);
		Order order = optional.orElseThrow(()->new OrderMsException("Order does not exist"));
